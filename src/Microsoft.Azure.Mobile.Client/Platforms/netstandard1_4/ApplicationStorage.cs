        ...
        bool IApplicationStorage.TryReadSetting(string name, out object value)
        {
            ...
            try
            {
                ...
                */
                ...
                using (FileStream fileStream = PlatformDetail.AppStorageOps.GetFileStream(string.Concat(this.StoragePrefix, name), FileMode.OpenOrCreate, FileAccess.Read))
                {
                    using (var reader = new StreamReader(fileStream))
                    {
                        value = reader.ReadToEnd();
        ...
        void IApplicationStorage.WriteSetting(string name, object value)
        {
            ...
            try
            {
                ...
                */
                ...
                using (FileStream fileStream = PlatformDetail.AppStorageOps.GetFileStream(string.Concat(this.StoragePrefix, name), FileMode.OpenOrCreate, FileAccess.Write))
                {
                    using (var writer = new StreamWriter(fileStream))
                        writer.WriteLine(value.ToString());
                }
